...
while(True):
    ...
    small_points = [[0, 0, 0] for _ in range(9)]
    for i in range(len(contours)):
        # 计算轮廓周长
        peri = cv2.arcLength(contours[i], True)
        # 进行多边形逼近
        points = cv2.approxPolyDP(contours[i], 0.01 * peri, True)
        #print(points)
        # 如果多边形有四个顶点，则认为是矩形
        if len(points) == 4:
            if i == 0:
                out_points = [[0, 0, 0] for _ in range(4)]
                for point in points:
                ...
                # 绘制矩形轮廓
                cv2.drawContours(img, [points], -1, (0, 255, 0), 1)
